...
from Uniddpm.ddpm.guided_diffusion.image_datasets import _list_image_files_recursively
...
class FeatureDataset(Dataset):
    ...
    def __getitem__(self, index):  # 索引参数 index
...
class ImageLabelDataset(Dataset):
    ...
    :param num_images: restrict a number of images in the dataset. 限制数据集中的图像数量。
    ...
    def __init__(
        ...
        self.image_paths = _list_image_files_recursively(data_dir)  # 递归地列出给定目录下的所有图像文件的路径，并将这些路径存储
        self.image_paths = sorted(self.image_paths)  # 对 self.image_paths 列表进行排序
    ...
    def __getitem__(self, idx):
        ...
        tensor_image = self.transform(pil_image)  # 对PIL图像对象pil_image应用了数据变换transformation,将其转换为PyTorch张量
        # tensor_image.shape：（3，256，256）因为图片是彩色的，所以是3通道的。

        # Load a corresponding mask and resize it to (self.resolution, self.resolution) 标签是.npy格式
        label_path = self.label_paths[idx]
        label = np.load(label_path).astype('uint8')  # （256，256）
        label = cv2.resize(

...
class ACDCDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = torch.from_numpy(np.array(img)).unsqueeze(0).float() / 255.0   #  （1，256，256）
...
class ACDC_FX_Dataset(Dataset):
    ...
    def __getitem__(self, item):
        id = self.ids[item]
        sample_path = os.path.join(self.root, id)
        fx_img = torch.load(sample_path)  
        fx_img = torch.squeeze(fx_img, dim=0) 

...
from dataset.acdc_fx import ACDC_FX_Dataset
...
parser.add_argument('--config', default='configs/acdc.yaml', type=str, required=False)
...
parser.add_argument('--log_file', default='', type=str,
                    required=False, help='Path to the log file')
parser.add_argument('--seed', type=int, default=0)
parser.add_argument('--ddpmf_id_path', default='', type=str, required=False)
...
def main():
    ...
    model = UNet(class_num=cfg['nclass'])
    ...
    trainset_fx = ACDC_FX_Dataset(name=cfg['dataset'], root=cfg['fx_data_root'], mode='train_u',
                             size=cfg['crop_size'], id_path=args.ddpmf_id_path)
    trainloader_fx = DataLoader(dataset=trainset_fx, batch_size=cfg['batch_size'], shuffle=True,
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        total_loss_ddpm_feature = AverageMeter()
        total_loss_w_fp = AverageMeter()
        total_mask_ratio = AverageMeter()

        loader = zip(trainloader_l, trainloader_u, trainloader_u_mix, trainloader_fx)  


        for i, ((img_x, mask_x),  
                (img_u_w, img_u_s1, img_u_s2, cutmix_box1, cutmix_box2),  
                (img_u_w_mix, img_u_s1_mix, img_u_s2_mix, _, _), img_feature) in enumerate(loader):
            ...
            pred_u_feature = model(img_feature)
            ...
            loss_x = (criterion_ce(pred_x, mask_x) +
                      criterion_dice(pred_x.softmax(dim=1), mask_x.unsqueeze(1).float())) / 2.0
            ...
            loss_u_ddpm_feature = criterion_dice(pred_u_feature.softmax(dim=1), mask_u_w.unsqueeze(1).float(),
                                         ignore=(conf_u_w < cfg['conf_thresh']).float())

            loss = (loss_x + loss_u_s1 * 0.2 + loss_u_s2 * 0.2 + loss_u_w_fp * 0.3 + loss_u_ddpm_feature * 0.3) / 2.0
            ...
            total_loss_ddpm_feature.update(loss_u_ddpm_feature.item())
            ...
            if rank == 0:
                ...
                writer.add_scalar('train/loss_u_ddpm_feature', loss_u_ddpm_feature.item(), iters)
                writer.add_scalar('train/mask_ratio', mask_ratio, iters)

            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info(
                    'Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss s: {:.3f}, Loss w_fp: {:.3f}, loss_u_ddpm_feature: {:.3f}, Mask ratio: '
                    '{:.3f}'.format(i, total_loss.avg, total_loss_x.avg, total_loss_s.avg,
                                    total_loss_w_fp.avg, total_loss_ddpm_feature.avg, total_mask_ratio.avg))
